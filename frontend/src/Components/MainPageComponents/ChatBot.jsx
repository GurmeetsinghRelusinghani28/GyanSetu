...
const ChatModal = ({ god, onClose }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { text: "⚠️ Unable to get response. Try again.", sender: "bot" },
      ]);
    }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 p-4">
      <div className="w-full sm:w-[90%] md:w-[70%] lg:w-[50%] xl:w-[40%] bg-white rounded-xl shadow-lg flex flex-col max-h-[90vh]">
        ...
        {/* Chat Body */}
        <div className="p-4 flex-1 overflow-y-auto flex flex-col space-y-3 bg-gray-100 text-sm sm:text-base">
          {messages.map((msg, index) => (
            <p
              key={index}
              className={`px-4 py-2 rounded-lg max-w-[80%] sm:max-w-[70%] break-words ${
                msg.sender === "user"
        ...
        <div className="flex items-center p-3 border-t bg-white rounded-b-xl">
          <input
            ...
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && sendMessage()}
            className="flex-1 px-3 py-2 bg-gray-200 text-black rounded-lg outline-none text-sm sm:text-base"
          />
          <button
            onClick={sendMessage}
